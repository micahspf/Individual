...
interface ShopClientProps {
  /** Tab id from ?cat=, already normalized by the server page. */
  initialCat: string;
  /** Search query from ?q=. */
  initialQ: string;
}

export default function ShopClient({ initialCat, initialQ }: ShopClientProps) {
  ...
  // For You reads localStorage, which the server render can't see. False on
  // the server and during hydration, true right after — keeps both first
  // renders identical, then real history fills in.
  ...
  const baseForTab = useMemo(() => {
    ...
    if (tab.id === 'for-you') return hydrated ? forYouProducts(who, shopProducts) : [];
    return shopProducts.filter((p) => p.category === tab.category);
  }, [activeTab, tab, who, hydrated]);
